...
export class ScrollView extends BaseScriptComponent {
  ...
  private defineScriptEvents() {
    ...
    this.createEvent("OnStartEvent").bind(() => {
      ...
      /**
       * ---------------------------------------------------------------------
       * Force the content to be slightly larger than the scroll area if it 
       * is smaller, so the user can scroll even with a minimal (e.g. 1 item) 
       * content.
       * ---------------------------------------------------------------------
       */
      if (this.contentLength < this.scrollAreaSize.y) {
        const forcedLength = this.scrollAreaSize.y + 0.01
        this.log.i(
          `Forcing minimum content length from ${this.contentLength} to ${forcedLength}`
        )
        this.contentLength = forcedLength
      }
